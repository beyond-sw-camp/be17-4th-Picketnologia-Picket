...
✔️ 데이터 일관성 유지
클러스터는 노드 간 데이터 동기화를 통해 예매 정보, 좌석 상태, 결제 기록 등 
중요한 데이터를 일관되게 유지한다.
예매 중 한 서버가 다운되더라도 다른 노드가 동일한 데이터를 보유하고 있어 
좌석 중복 예매, 결제 누락 같은 문제를 방지할 수 있다.

✔️ 부하 분산
인기 공연/영화 티켓 오픈 시 대량 트래픽이 몰리는 상황에도 
여러 노드가 요청을 분산 처리한다.
이를 통해 서버 과부하로 인한 예매 실패나 
대기 시간 증가 없이 안정적인 서비스 제공이 가능하다.

✔️ 쓰기 작업에 최적화된 구조
티켓팅 서비스는 좌석 선택, 결제, 예매 완료 등 
실시간 쓰기 작업이 빈번한 구조다.
클러스터는 다수의 노드가 병렬로 쓰기 요청을 처리하여 
빠른 응답성과 높은 처리량을 제공한다.

✔️ 고가용성 (High Availability)
특정 노드가 장애를 일으켜도 HAProxy 등 로드 밸런서를 통해 
다른 노드로 자동 전환되어
예매 중단 없이 안정적인 운영이 가능하다. 
중요한 서비스일수록 장애 대응이 필수다.

✔️ 유연한 확장성
티켓팅 수요가 늘어날 경우 노드를 손쉽게 추가하여 
트래픽 처리 용량을 수평 확장할 수 있다.
이벤트성 대형 공연, 연말 시즌 등 피크 트래픽에도 
유연하게 대응 가능하다.
